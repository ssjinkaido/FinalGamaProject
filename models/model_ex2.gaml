...
experiment exp_2 type: gui until: (nb_butterfly=0) or (cycle > 200) {
...
experiment exp_2_batch type: batch repeat: 16 keep_seed: true until: (nb_butterfly=0) or (cycle > 200) {
	float seed <- seed_value;
	permanent {
		display batch_display type:2d{
			chart "Butterfly population chart" type: series 
				x_serie_labels: ("horizontal l->r") x_label: 'Type of color transition'{									
